...
static void print_help(void)
{
	...
	printf("  -d, --device-name=NAME:\tSet evdev device name to NAME\n");
	printf("\n");
	print_bias_help();
	printf("\n");
...
struct mon_ctx {
	...
	unsigned int buttons[GPIOD_LINE_BULK_MAX_LINES];
};
			 ...
			 const struct timespec *timestamp)
{
	...
	ctx->events_done++;
	if (libevdev_uinput_write_event(ctx->uidev, EV_KEY, ctx->buttons[line_offset - 1], (event_type == GPIOD_CTXLESS_EVENT_CB_RISING_EDGE) ? 1 : 0) != 0) {
		die_perror("libevdev_uinput_write_event(EV_KEY) failed");
	}
	if (libevdev_uinput_write_event(ctx->uidev, EV_SYN, SYN_REPORT, 1) != 0) {
		die_perror("libevdev_uinput_write_event(EV_SYN) failed");
	}
}
...
int main(int argc, char **argv)
{
	...
	libevdev_set_name(dev, "gpio2uinput controller");
	...
	for (i = 1; i < argc; i++) {
		...
		offsets[i - 1] = offset;
		libevdev_enable_event_code(dev, EV_KEY, button, NULL);
		ctx.buttons[offset - 1] = button++;
		num_lines++;
	}

	ctx.sigfd = make_signalfd();

	if ((err = libevdev_uinput_create_from_device(dev, LIBEVDEV_UINPUT_OPEN_MANAGED, &(ctx.uidev))) != 0) {
